...
def main():
    ...
    num_keys = len(descriptor)-1
    print("Number of keys: " + str(num_keys))

    key_info = []
    for i in range(num_keys):
        d = descriptor[i+1].split('[')[1].split('*')[0] + '*'
        print(f"This round {d}")

        new = {
            "fingerprint": d.split(']')[0][0:8],
            "xpub": d.split(']')[1].split('/')[0],
            "derivation": 'm/'+d.split(']')[0][9:].replace('h','\' ')
        }
        print( new )
        key_info.append( new )

        key_info_qrcode = qrcode.make( d )
        key_info_qrcode.save(f"qrcodes/key_info_{i+1}_qrcode.png")
    ...
    for i, key in enumerate(key_info):
        ...
        c.drawCentredString(key_num_x_loc+key_size/2 + key_num_box_width/2, key_num_y_loc+ text_box_height/4.5, str(i+1) + f"/{num_keys}")
        ...
        c.drawCentredString(fp_x_loc+fp_size/2+ (fp_box_width)/2,fp_y_loc + text_box_height/4.5, key['fingerprint'])
    ...
    for i, key in enumerate(key_info):
        ...
        c.drawCentredString(border_width+object_padding+title_box_width/2, -top_border_width-top_padding-text_box_height+ text_box_height/4.5,"#"+str(i+1)+" xpub")

        # Add xpub qr code
        c.drawImage(f"qrcodes/key_info_{i+1}_qrcode.png", qr_x_loc-.025*qr_size,qr_y_loc-.025*qr_size,width = qr_size*1.05,height=qr_size*1.05)
